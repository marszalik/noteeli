...
| Kanban — parent card locked in its column until all its subtasks leave that column | ✅ e2e `test_parent_locked_until_subtasks_moved` | `kanbanCardLocked` cross-column rule |
| Kanban — Jira-style persistent parent link: subtask moved to another column keeps its relation (`^id` block id on parent, `[[#^id]]` ref on child — native Obsidian syntax), shows `↳ parent` chip, counts toward the parent's progress; re-nesting removes the markers | ✅ e2e `test_detached_subtask_keeps_parent_link` | raw `card.text` keeps markers; `kanbanCardMeta` parses on demand |
...
| Kanban — no dedicated file type: the topbar mode button cycles WYSIWYG → Markdown → Kanban → Text; picking Kanban on a plain note converts it (headings → columns, empty note → 3 default columns), frontmatter written only with the first real board edit | ✅ e2e `test_mode_toggle_converts_plain_note_to_board` | deliberate — creation stays file/dir only; detection purely content-based (`isKanbanContent`) |
| Markdown files: "Text" (bare CodeMirror) and "Code" (markdown-highlighted CodeMirror) views in the mode dropdown — five views total | ✅ e2e `test_board_renders_columns_cards_and_subtask_thread` (menu + token assertions) | `text_mode` / `plain_mode`; save path reads from the active view |
| Mode switching alone never dirties/saves the document (Toast UI `changeMode` change-event guarded) | ✅ e2e `test_mode_toggle_converts_plain_note_to_board` | `isApplyingDocument` wrap in `setEditorMode` |
| Per-file view memory — last picked view restored on reopen, survives page reload; kanban frontmatter always wins over it | ✅ e2e `test_mode_toggle_converts_plain_note_to_board` | `localStorage["noteeli-file-view-modes"]`, pruned to 200 entries |
| WYSIWYG preserves `---` frontmatter (Toast UI `frontMatter: true`); `***`-mangled board files from older versions still detected and healed on first board edit | ✅ e2e `test_wysiwyg_roundtrip_preserves_frontmatter`, `test_mangled_frontmatter_board_recognized_and_healed` | `isKanbanContent` accepts `***` fences; `ensureKanbanFrontmatter` heals |
| Kanban drag & drop shows a card-sized ghost placeholder at the exact landing spot; drops between cards insert at that position | 🌐 (drag e2e tests exercise the drop path) | `kanbanContainerDropSpot`, `.kanban-drop-placeholder` |
...
| Auto-push after checkpoints; clean divergence rebased, conflicts parked non-destructively | ✅ `tests/test_git_autopush.py` (6 tests: push, rebase-replay, conflict-park, parked outcome, no-remote no-op, flag off) | `NOTEELI_GIT_AUTOCOMMIT_PUSH=1`; `GitService.sync_push` (push → pull --rebase → push; abort+park on conflict) |
